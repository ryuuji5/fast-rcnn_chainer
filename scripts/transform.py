...
class Transform(object):
    ...
    def transform(self, datum, datadir, train=True):
        img_fn = '%s/images/%s' % (datadir, datum[1])
        self._img = cv.imread(img_fn)
        self.orig = self._img.copy()
        self._joints = np.asarray([int(float(p)) for p in datum[3:31]])
        ...
        joints[:, 0] /= w
        joints[:, 1] /= h
        #self._joints = np.hstack([joints.flatten(), np.asarray(datum[2])])
    ...
    def crop(self):
        ...
        self._aspect_ratio = np.float32(h) / np.float32(w)
    ...
    def fliplr(self):
        if np.random.randint(2) == 1 and self.flip == True:
            ...
            # ankle
            joints[0], joints[5] = joints[5], joints[0]
            # knee
            joints[1], joints[4] = joints[4], joints[1]
            # hip
            joints[2], joints[3] = joints[3], joints[2]
            # shoulder
            joints[8], joints[9] = joints[9], joints[8]
            # elbow
            joints[7], joints[10] = joints[10], joints[7]
            # wrist
            joints[6], joints[11] = joints[11], joints[6]
            self._joints = np.array(joints).flatten()

    def revert(self, img, pred):
        c, w, h = img.shape
        center_pt = np.array([w / 2, h / 2])
        joints = np.array(zip(pred[0::2], pred[1::2]), dtype = np.float32)  # x,y order
        joints[:, 0] *= w
        joints[:, 1] *= h
        joints += center_pt
