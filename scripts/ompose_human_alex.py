...
if __name__ == '__main__':
    ...
    for i, datum in enumerate(test_dl):
        ...
        datum = datum.split(',')
        im_name = datum[0]
        EB_rects = open('%s/data/EB_normal/%s'%(OP_PATH, im_name.replace('png','csv'))).readlines()
        ...
        result = draw_result(crop_image, clss, bbox, rects, args.nms_thresh, args.conf, im_name, gt)
        cv.imwrite('output/alex_normal/%s'%datum[0], result)
    pbar.finish()
